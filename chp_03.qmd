...
![Hierarchical model for the football players example. Notice how we have one more level than in previous hierarchical models](/fig/beta_binomial_hierarchical_subjects_dag.png){#fig-beta_binomial_hierarchical_subjects_dag width=50%}
...
with pm.Model(coords=coords) as model_football:
    # Hyper parameters
    mu = pm.Beta('mu', 1.7, 5.8)  # a maximum entropy prior with 95% of the mass between 0 and 0.5 
    nu = pm.HalfNormal('nu', 100) # a vague prior for nu (the concentration of the beta distribution)
    ...
    nu_p = pm.HalfNormal('nu_p', 100, dims = "pos")
...
1. Repeat the exercise we did with `model_h`. This time, without a hierarchical
structure, use a flat prior such as $\text{Beta}(\alpha=1, \beta=1)$. Compare the results of both models.

2. Create a hierarchical version of the tips example from @sec-programming_probabilistically by partially pooling across the days of the week. Compare the results to those obtained without the hierarchical structure.

3. Amino acids are usually grouped into the categories like `polar`, `non-polar`, `charged`, and `special`. Build a hierarchical model like `cs_h` but that includes a group effect for the amino acid category. Compare the results to those obtained in this chapter.
