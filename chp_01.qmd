...
Being able to map symbols like, ⚀, or strings like "yes" to numbers, makes analysis simpler as we already know how to do math with numbers. Random variables are also useful because we can operate with them without directly thinking in terms of the sample space. This feature becomes more and more relevant as the sample space becomes more complex. For example, when simulating molecular systems we need to specify the position and velocity of each atom, for complex molecules like proteins this means that we will need to track thousands, millions or even larger numbers. Instead, we can use random variables to summarize certain properties of the system, such as the total energy or the relative angles between certain atoms of the system. If you are still confused, that's fine, the concept of a random variable may sound too abstract at the beginning, but we will see plenty of examples throughout the book that will help you cement these ideas. Still, let me try one analogy that I hope you find useful. Random variables are useful in a similar way that Python functions are useful. We often encapsulate code within functions, so we can store, reuse and "hide" complex manipulations of data into a single call. Even more, once we have more than one function we can, sometimes, combine them differently, like using adding the output of two functions or using the output of one function as the input of the other. We can do all this without functions, but abstracting away the inner workings, not only makes the code cleaner it also helps with understanding and fostering new ideas.

![random variable](/fig/random_variable.png){#fig-random_variable}

The mapping between the sample space and $\mathbb{R}$ is deterministic. There is no randomness involved. So why do we call it a *random* variable? Because we can *ask* the variable for values, and every time we ask, we will get a different number. The randomness comes from the probability associated with the events, in 
@fig-random_variable we have represented $P$ as the size of the circles. Given that we have random variables that are functions and a bunch of probabilities associated with them. What about putting all these probabilities into a single object like a function? That's the topic of the next section.

The two most common types of random variables are discrete and continuous ones. Without going into a proper definition we are going to say that discrete variables take only certain values, we usually use integers to represent them, like 1, 5, 42.  And continuous variables take real values, so we use floats to work with them, like 3.1415, 1.01, 23.4214, etc

When we need one or the other is problem-dependent. If we ask people about their favorite color we will get answers like "red", "blue", and "green". This is an example of a discrete random variable. The answers are categories, there are no intermediate values between "red" and "green". But if we are studying the properties of light absorption, then discrete values like "red" and "green" may sound not very adequate and instead working with wavelength seems more appropriate, in that case, we will expect to get values like 650 nm and 510 nm and an infinite number of values between them.
...
Empirical distributions are very useful, and we are going to extensively use them. But instead of rolling dice by hand we are going to use advanced computational methods to make the hard work for us, this will not only save us time and boredness by it will allow us to get samples from really complicated distributions effortlessly. But we are getting ahead of ourselves. We know what to focus on theoretical distributions, which are central in statistics among other reasons because they allow the construction of probabilistic models.

As we saw, there is nothing random or mysterious with random variables, they are just a type of mathematical function. The same goes for theoretical probability distributions. I like to compare probability distributions with circles. Because we are all familiar with circles even before we get into school, we are not afraid of them and they don't look mysterious to us. We can define a circle as the geometric space of points on a plane that is equidistant from another point called the center. And we can even give a mathematical expression for this definition. In an $x-y$ Cartesian coordinate system, the circle with radius $r$ is the set of all points $(x, y)$ such that

$$x^2 + y^2 = r^2 $$

We are omitting here the fact that we can move the circle around the coordinate system. We don't care about that.

Given the **parameter** $r$, the radius, of a circle is completely defined. It is all that we need to plot it and all that we need to compute properties such as the perimeter, $2\pi r$. Because all circles are essentially very similar objects, we can think of them as The Family of Circles, where each member is identified from the rest from the value of $r$.

All of these directly apply to probability distributions. They also have mathematical expressions that define them, and these expressions have parameters, that we can change to define all members of a family of probability distributions. @fig-dice_distribution shows four members of one probability distribution known as beta-binomial.
...
you will get an interactive plot that will allow you to explore BetaBinomial distribution by changing `alpha`, `beta` and `n`. @fig-beta_binomial_interactive shows a static version of the interactive plot.

![The output of pz.BetaBinomial(alpha=10, beta=10, n=6).plot_interactive(). We got 3 sliders to interactively change the parameter of the distribution](/fig/beta_binomial_interactive.png){#fig-beta_binomial_interactive}


 On the x-axis we have the support of this distribution, i.e. the values it can take, $x \in \{0, 1, 2, 3, 4, 5\}$. On the y-axis, the probabilities are associated with each of those values. Tabulated, they will be:
...
Notice that the black dots represent the probabilities for each value of the random variable, while the dotted black line is just a visual aid. The `BetaBinomial(alpha=10, beta=10, n=6)` distribution will return 0 for any value not in $\{0, 1, 2, 3, 4, 5\}$, including values such as $-1, 0.5, \pi, 42$. 

We previously mentioned that we can *ask* for a random variable for values and every time we ask, we will get a different number. We can simulate this with PreliZ, for instance:
...
Notice that now we wrote $pdf$, meaning probability density function, instead of $pmf$. Why the difference? Let's take one step back, the output of a discrete probability distribution is a probability, where the height of the bars in @fig-dice_distribution or the height of the dots in @fig-beta_binomial_interactive are probabilities. Each bar or dot will never be higher than 1 and if you sum all the bars or dots you will always get 1. Let's do the same now but with the curve in #fig-gaussian_pdfs. We can think that the curve is made up of super thin bars, so thin that we assign one bar for every real value in the support of the distributions, we measure the height of each bar and we perform an infinite sum. Will this sum give as exactly 1? or a large number? Is the sum actually finite or do we get infinite?

A proper answer to this question requires measure theory, and we are not going into that rabbit hole. But the answer is that we can only assign a probability of 0 to every possible value a continuous random variable can take, and we can only mindfully compute probabilities if we ask about the range of values. For a Gaussian, the probability of getting exactly the number -2, i.e the number -2 follow by an infinite number of zeros after the decimal point, is 0. But the probability of getting a number between -2 and 0 is some positive number. The practical take-away message is that $pdf$ return densities and to get a probability from a probability density we must integrate over some interval:
...
You may remember that we can approximate an integral by summing areas of rectangles and the approximation becomes more and more accurate as we reduce the length of the base of the rectangles[^2]. Based on this idea and using PreliZ we can estimate $P(-2 < X < 0)$ as:
...
We have seen the $pmf$ and the $pdf$, another useful way to characterize distributions is to use the cumulative distribution function. The $cdf$ of a random variable $X$ is the function $F_X$ given by $F_X(x) = P(X \le x)$. In words, the $cdf$ is the answer to the question of what is the probability of getting a number lower or equal to $x$. #fig-cdf_pdf shows the on the first column the $pmf$ and $cdf$ of a Betabinomial and on the second column the $pdf$ and $cdf$ of the Gaussian. Notice how the $cdf$ has *jumps* for the discrete variable but it is smooth for the continuous. The height of each *jump* represents a probability, just compare them with the height of the dots. We can use the plot of the $cdf$ of a continuous variable as visual proof that probabilities are zero for any value of the continuous variable.
...
Just by looking at a $cdf$, it is easier to get what is the probability of getting a number smaller than 1, we just need to go to the value 1 on the x-axis, move up until we cross the black line and then check the value of the y-axis. Even without a ruler or gridlines, we can see that it should be something in the middle of 0.75 and 1, let's say it is $\approx 0.85$. This is way harder to do with the $pdf$ because for the pdf we need to compare the entire area below 1 to the total area to get the answer. Judging areas is more difficult than height/length.  
...
On the first subplot of @fig-beta_binomial_posterior, we have zero trials, thus the three curves represent our
priors:
...
* Remember that infinite is a limit and not a number, so from a practical point of view, we could get practically indistinguishable posteriors for a finite and rather small number of data points.
...
From the preceding example, it is clear that priors can influence inferences. That's fine, priors are supposed to do this. Maybe it would be better to not have priors at all. That would make modeling easier, right? Well, not necessarily. If you are not setting the prior someone else will be doing it for you. Sometimes this is fine, as the "default prior", could be useful, sometimes it is better to have more control. Let me explain, we can think that every model, Bayesian or not, has some kind of prior in one way or another, even if the prior is not set explicitly. Many results from frequentist statistics, can be seen as special cases of a Bayesian model under certain circumstances, such as flat priors. One common frequentist method to estimate parameters is known as maximum likelihood; this method avoids setting a prior and works just by finding the value that maximizes the likelihood. This value is usually notated by adding a little hat on top of the symbol of the parameter we are estimating, such as $\hat \theta$ or sometimes $\theta_{\text{mle}}$ (or even both). $\hat \theta$ is a point estimate (a number) and not a distribution. For the coin-flipping problem, we can compute this analytically:
$$
\hat \theta = \frac{y}{N}
$$

If you go back to  @fig-beta_binomial_posterior, you will be able to check for yourself that the mode of the blue posterior (the one corresponding to the uniform/flat prior) agrees with the values of $\hat \theta$, computed for each subplot. So, at least for this example, we can see that even when the maximum likelihood method does not explicitly invoke any prior, it can be considered a special case of a Bayesian model, one with a uniform prior.

We cannot avoid priors, but if we include them in our analysis, we will get some potential benefits, including a distribution of plausible values and not only the most probable one. Having a distribution of value can be more informative, as we will have an estimate of the uncertainty. This can lead to models that are more difficult to overfit and predictions that are more robust. 

Sometimes priors can even make inference easier or faster.


Another advantage of being explicit about priors is that we get more transparent models, meaning they're easier to criticize, debug (in a broad sense of the word), and hopefully improve. 

Also having to think about priors can make us think about the problem we are trying to solve and the data we have. In short, priors are not a nuisance, they are a tool that can help us in our analysis.




Building models is an iterative process; sometimes the iteration takes a few minutes, and sometimes it could take years. Sometimes it will only involve you, and sometimes it will involve people you do not even know. Reproducibility matters and transparent assumptions in a model contribute to it. Besides, we are free to use more than one prior (or likelihood) for a given analysis if we are not sure about any special one; exploring the effect of different priors can also bring valuable information to the table. Part of the modeling process is about questioning assumptions, and priors (and likelihood) are just that. Different assumptions will lead to different models and probably different results. By using data and our domain knowledge of the problem, we will be able to compare models and, if necessary, decide on a winner. Chapter 5, Model Comparison, will be devoted to this issue. Since priors have a central role in Bayesian statistics, we will keep discussing them as we face new problems. So if you have doubts and feel a little bit confused about this discussion, just keep calm and don't worry, people have been confused for decades and the discussion is still going on.
...
![A Kruschke's diagram of a beta-binomial model](/fig/beta_binomial_dag.png)
...
ArviZ is a Python package for exploratory analysis of Bayesian models, and it has many functions to help us summarize the posterior. One of those functions is
`az.plot_posterior`, which we can use to generate a plot with the mean and HDI of. The distribution does not need to be a posterior distribution will work. Figure @fig-plot_posterior_beta shows the result for a random sample from a beta distribution:
...
![The 94% HDI for a sample from a beta distribution](/fig/plot_posterior_beta.png){#fig-plot_posterior_beta}
...
We do not know whether the brain works in a Bayesian way, in an approximately Bayesian fashion, or maybe some evolutionary (more or less) optimized heuristics. Nevertheless, we know that we learn by exposing ourselves to data, examples, and exercises... well you may say that humans never learn, given our record as a species on
subjects such as wars or economic systems that prioritize profit and not people's well-being... Anyway, I recommend you do the proposed exercises at the end of each chapter:
