...
![random variable. UPDATE](/fig/random_variable.png){#fig-random_variable width=80%}
...
Given the **parameter** $r$ the circle is completely defined. It is all that we need to plot it and all that we need to compute properties such as the perimeter, which is $2\pi r$. Because all circles are essentially very similar objects, we can think of them as The Family of Circles, where each member is set apart from the rest precisely by the value of $r$.
...
![We got 3 sliders to interactively change the parameter of the distribution. This is the output of `pz.BetaBinomial(alpha=10, beta=10, n=6).plot_interactive()`. ](/fig/beta_binomial_interactive.png){#fig-beta_binomial_interactive width=80%}
...
Visualizations are usually easier to read than tables. Notice that the black dots in @fig-beta_binomial_interactive represent the probabilities for each value of the random variable, while the dotted black line is just a visual aid. The `BetaBinomial(alpha=10, beta=10, n=6)` distribution will return 0 for any value not in $\{0, 1, 2, 3, 4, 5\}$, including values such as $-1, 0.5, \pi, 42$.
...
A proper answer to these questions requires measure theory, and this is a very informal introduction to probability, so we are not going into that rabbit hole. But the answer essentially is that for a continuous random variable, we can only assign a probability of 0 to every individual value it may take; instead, we can assign densities to them and then we can calculate probabilities for a range of values. Thus, for a Gaussian, the probability of getting exactly the number -2, i.e the number -2 follow by an infinite number of zeros after the decimal point, is 0. But the probability of getting a number between -2 and 0 is some number larger than 0 and smaller than 1. To find out the exact answer we need to compute:
$$
P(a < X < b) =  \int_a^b \text{pdf}(x) dx
$$

And to compute that we need to replace the symbols for concrete quantity. If we replace the pdf by $\text{Normal}(0, 1)$, and $a=-2$, $b=0$, we will get that $P(-2 < X < 0) \approx 0.477$, this is the shaded area in @fig-gauss_prob.

![The black line represent the pdf of a Gaussian with parameters mu=0 and sigma=1, the gray area is the probability of a value being larger than -2 and smaller than 0](/fig/gauss_prob.png){#fig-gauss_prob width=80%}
...
The central panel of @fig-joint_marginal_cond represents $p(A, B)$ using a grayscale with darker colors for higher probability density. We see the joint distribution is elongated, indicating that a longer earthworm is likely to weigh more than a short one. Knowing the value of $A$ tells us something about the values of $B$ and the other way around. On the top and right *margins* of @fig-joint_marginal_cond we have the **marginal distributions** $p(A)$ and $p(B)$ respectively. To compute the marginal of $A$ we take $p(A, B)$ and we average overall values of $B$, intuitively this is like taking a 2D object, the joint distribution, and projecting it into one dimension. The marginal distribution of $B$ is computed similarly. The dashed lines represent the **conditional probability** $p(A \mid B)$ for 3 different values of $B$. We get them by slicing the joint $p(A, B)$ at a given value of $B$. We can think of this as the distribution of $A$ given that we have observed a particular value of $B$.

![The relationship between the join, the marginal and the conditional probabilities](/fig/joint_marginal_cond.png){#fig-joint_marginal_cond width=80%}
...
![A Kruschke's diagram of a beta-binomial model](/fig/beta_binomial_dag.png){width=20%}
