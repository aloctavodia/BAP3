...
When working with PyMC we saw that sampling from the posterior predictive distribution, at not observed values, requiere us to define the "Xs" as  `Mutable data` and then update the variable before computing the posterior predictive distribution. With Bambi this is not necessary, we can use the `predict` method to predict at new values by passing the new values to the `data` argument. For example, let's predict the length of a baby at 0.5 months (15 days):
...
Variable selection refers to the process of identifying the most relevant variables in a model from a larger set of potential predictors. We perform variable selection under the assumption that only a subset of variables have a considerable impact on the outcome of interest while others contribute little or no additional value. Arguably the "most Bayesian thing to do" when modeling is to include all variables that we may thing of in a single model and then use the posterior from that model to make predictions or gain understanding in the relationships of the variables. This is the "most Bayesian" approach because we are using as much data as posible and incorporating in the posterior the uncertainty about the importance of the variables. However, being more Bayesian than Bayes is not always the best idea. Variable selection is a good idea when

* We need to reduce the measurment cost. For instance in medicine we may have the money and resources to run a pilot study and measure 30 variables for 200 patients. But we can not do the same from thousands. Or we may be able to place a lot of sensors in an open field to better model crop gains, but we can not extend that to the size of a country. Reducing cost is not always about money or time, when working with humnans or other animales reducing pain, disconfort is important. For example, we may want to predict the risk of a patient to have a heart attack, we can do this by measuring a lot of variables, but we can also do it by measuring just a few variables that are less invasive.
* We want to reduce the computational cost. This is not a problem for small and simple models, but when we have a lot of variables, or a lot of data, or both, the computational cost can be prohibitive.
* We seek a better understanding of significant correlation structures. That is we are interesting in understading which variables provide better predictions. It is important to remark here that we are not talking about causuality here. While statistical models, in particular GLMS can be used to infer causality, doing so requiere extra steps and assumptions. In this book we do not discuss how to perform causual inference. For a very gentle introduction to causual inference please check this video <https://www.youtube.com/watch?v=gV6wzTk3o1U> and if you are more serious you can check the online book Causal Inference: The Mixtape by Scott Cunningham [@cunningham2021causal] <https://mixtape.scunning.com/>.
* When we desire a model that is more resilient to changes in the data-generating distribution, that is we can see variable selection as a method to make model mroe robuts against unrepresentative data.

### Projection predictive inference

There are many methods to perform variable selection. In this section we will focus on one of them called Projection Predictive Inference [@Piironen2020, @mclatchie2023]. The main reason we are focusing on this single method is that it has shown very good performance across a broad range of fields.

The main steps of Projective Prediction are:

1. Generate a reference-model, i.e. a model with all the variables you are happy to include.
2. Generate a set of submodels, i.e. models with a subset of the variables.
3. Project the reference-model's posterior distrivution into the submodels
4. Pick the smallest model that make predictions as similar as the referennce-model

For this procedure we only perform Bayesian inference once, for the reference model. For the submodels we do not explicitelly compute the posterior, instead the posterior disribution of the referrence-model is projected into the submodel. We are going to ommit how this is done, but intuitivelly what we need to do is to find the parameters for the submodel that generate posterior predictive distribution that match as close as posible the predictions generated for the reference-model. The projection can be done in a computationally efficient way so the cost of generating the submodels is order of magnitude faster compared to explicitelly computing their posteriors. Additionally, we only need to provide priors for the reference-model, and we only need to diagnose the samples from the reference mdoel. The submodels can be then constructed automatically, so this procedure also saves human time.

One resons projective predictions works in practice is thanks to the use of a reference-model. By fitting the submodels to the in-sample predictions made by the reference model, instead of the observed data, we are filtering out the noise in the data. This help separate the more relevant variables from the less ones. Another factor is the use of cross-validation in the selection of the submodels (we discuss cross-validation in Chapter @sec-model_comparison).

### Projection predictive with Kulprit

Kulprit is a Python package for projection predictive inference. It works with Bambi, as we can pass a reference model built with it and Kulprit will do the hard work for us. To exemplify how to use Kulprit we are going to use the body fat dataset [@penrose1985]. This dataset has measurements from 251 individuals, including their weights, many body measurements like the circunference at the abodomen, biceps etc, and ages. Our purpose is to predict lean body weight. Since obtaining accurate measurements of body fat is expensive and potentially annoying for patients, we want to reduce the measurements while keeping a good predictive accuracy got the lean body weight.

First we need to define a reference model. 



For example, let's fit a model to the bikes data. Kulprit is under heavy development and still does not support splines. So instead of using them we are going to transform the `hour` variable into a two variables, the `sin(hour)` and `cos(hour)` this is a simple trick to model periodic data. 

```python
bikes["hour_sin"] = np.sin(bikes.hour * np.pi / 12)
bikes["hour_cos"] = np.cos(bikes.hour * np.pi / 12)

data = bikes[["rented", "hour_sin", "hour_cos", "temperature", "humidity", "windspeed"]]